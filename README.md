...
3. Summarise the transcript into chaptrs, one timestamped line per moment
4. Search and star the ones worth keeping
5. Click one and DaVinci Resolve jumps to that frame

A hundred hours takes roughly two hours unattended. Every stage resumes, so
...
ffmpeg, whisper.cpp and llama.cpp are bundled. The app updates itself when a new
release is published.

On first run open Settings and download a speech model and a language model,
around 10 GB depending on the tier you pick.
...
Go to Settings, DaVinci Resolve, Install script. Then in Resolve run Workspace,
Scripts, Chaptr Link, and leave it running. Clicking a chaptr or a transcript
line moves the playhead.
...
`chaptr-cli` runs the same pipeline headless, which is the practical way to
drive the long stages:
...
Early. Windows support is new and lightly tested. Linux packaged builds
transcribe on CPU for now; Windows uses Vulkan.
